...
HexCoordConverter::HexCoordConverter(QObject *parent)
    : QObject(parent), m_hexRadius(50) {
  // Pre-calculate common hex dimensions for flat-top hexes
  m_hexWidth =
      m_hexRadius * qSqrt(3);    // Side-to-side distance (w = r * sqrt(3))
...
QPointF HexCoordConverter::hexToPixel(int row, int col) const {
  ...
  // You might want to apply a global offset here if you want the board to be
  // centered around (0,0) or start from a specific corner. For now, we'll let
  // QML handle the centering of the entire board.
  return QPointF(x, y);
}