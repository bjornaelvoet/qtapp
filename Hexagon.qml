...
Item {
    ...
    property real hexSize: coordConverter.hexRadius
    ...
    Path {
        ...
        PathLine { x: hexSize * Math.cos(Math.PI * (0/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (0/3.0) + Math.PI / 6.0) } // Angle adjusted to start at top-left vertex for flat top
        PathLine { x: hexSize * Math.cos(Math.PI * (1/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (1/3.0) + Math.PI / 6.0) }
        PathLine { x: hexSize * Math.cos(Math.PI * (2/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (2/3.0) + Math.PI / 6.0) }
        PathLine { x: hexSize * Math.cos(Math.PI * (3/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (3/3.0) + Math.PI / 6.0) }
        PathLine { x: hexSize * Math.cos(Math.PI * (4/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (4/3.0) + Math.PI / 6.0) }
        PathLine { x: hexSize * Math.cos(Math.PI * (5/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (5/3.0) + Math.PI / 6.0) }
        PathLine { x: hexSize * Math.cos(Math.PI * (6/3.0) + Math.PI / 6.0); y: hexSize * Math.sin(Math.PI * (6/3.0) + Math.PI / 6.0) } // Close the path
    }
    ...
    states: [
        State {
            ...
            PropertyChanges { target: hexShape; strokeColor: "#2ecc71"; strokeWidth: 4 } // Green border
        }
    ]
    transitions: [
        Transition {
            from: "*"; to: "hovered"
            PropertyAnimation { properties: "strokeWidth,strokeColor"; duration: 100 }
        },
        Transition {
            from: "hovered"; to: "*"
            PropertyAnimation { properties: "strokeWidth,strokeColor"; duration: 100 }
